...
static void	gsh_r_cut_line(uintmax_t d, char *out, t_pos *pos)
{
	size_t size;
	...
	if (d == K_SHFT_UP)
	{
		...
		pos->kur += size;
		pos->len += size;
		gsh_r_redraw_line(out, pos, size);
